/*
  ...
  Editted for Nano 33 BLE Sense REV2 - 19 Jan 2024
*/
...
bool Drone_sensor::init() {
  ...
  return(true);  // only true if all (rtc, sd) begin successfully
}
...
bool Drone_sensor::laser( bool laserState ) {
    if(_laserState != laserState){                  // only do something if the input is different from the current laser state
      ...
      if ( _laserState ) {
          NRF_GPIOTE->TASKS_SET[0] = 1;             // set pin HIGH to turn laser on - updated 11/6/23 by ZK; L293D (with inverting output) replaced with NPN MOSFET (with non-inverting output)
      } else {
          NRF_GPIOTE->TASKS_CLR[0] = 1;             // clear/set pin LOW to turn laser off - updated 11/6/23 by ZK
      }
