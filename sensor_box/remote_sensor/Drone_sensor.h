       ...
       // ADC_channel* channelList[9] = { 0 };
        ...
        float getData(ADC_channel<NN>* channel) {
            ...
            for (int i = 0; i < channel->resultSize; i++) {
                ...
                channel->results[i] = float(oversampleSum) * channel->uVperLSB / channel->oversample; // return a voltage in uV
            ...
            result = resultSum * channel->uVperLSB / channel->resultSize / channel->oversample; // returns a value in uV
            return(result);
        };
